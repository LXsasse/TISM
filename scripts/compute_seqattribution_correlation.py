...
from drg_tools.stats_functions import correlation, mse
...
if __name__ == '__main__':
    ...
    filea = sys.argv[1] # sequence attributions 1
    fileb = sys.argv[2] # sequence attributions 2
    ...
    # Different ways to normalize data if not done beforehand
    ...
    # Provide sequence onehot encodings to compute correlations at the reference bases only
    ...
    # Select the output tracks if the model is a multi-task model
    ...
    for e, exp in enumerate(expcom):
        ...
        if not '--print_summary' in sys.argv:
    ...
    ## IF save npz, saves all experiments in same file instead of individual txt files for each experiment
    if '--savenpz' in sys.argv:
        np.savez_compressed(outname+'.npz', names = common, values = np.array(allcorr).T , experiments = expcom)
    if '--print_summary' in sys.argv:
